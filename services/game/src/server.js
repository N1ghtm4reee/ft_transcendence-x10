...
fastify.post("/api/game/accepted/:gameId", async (request, reply) => {
  try {
    ...
    return reply.send({
      success: true,
      message: "Game invitation accepted successfully",
      gameId: gameId,
    });
  } catch (error) {
...
function startGameLoop(gameId) {
  if (gameLoops.has(gameId)) {
    console.log(`Game loop for ${gameId} already exists`);
    return;
  }
  ...
  const session = sessions.get(gameId);
  if (!session) {
    console.log(`No session found for game ${gameId} when starting loop`);
    return;
  }
  ...
  const gameLoop = setInterval(() => {
    ...
    if (!currentSession || !currentSession.gameStarted) {
      console.log(
        `Game ${gameId} not started or session missing, pausing loop`
      );
      return;
    }

    // Use the imported WebSocket constant
    const p1Sock = currentSession.player1Sock;
    const p2Sock = currentSession.player2Sock;

    const p1Connected = p1Sock && p1Sock.readyState === WebSocket.OPEN;
    const p2Connected = p2Sock && p2Sock.readyState === WebSocket.OPEN;

    // Add detailed logging for connection status
    if (!p1Connected || !p2Connected) {
      console.log(`Game ${gameId} connection check failed:`, {
        p1Exists: !!p1Sock,
        p1ReadyState: p1Sock?.readyState,
        p1Connected: p1Connected,
        p2Exists: !!p2Sock,
        p2ReadyState: p2Sock?.readyState,
        p2Connected: p2Connected,
        WebSocketOPEN: typeof WebSocket !== "undefined" ? WebSocket.OPEN : 1,
      });

      // Don't pause immediately - give a few frames for connection to stabilize
      if (!currentSession.disconnectionWarning) {
        currentSession.disconnectionWarning = true;
        currentSession.disconnectionCount = 0;
      }

      currentSession.disconnectionCount++;

      // Only pause after several consecutive failed checks
      if (currentSession.disconnectionCount > 3) {
        console.log(
          `Game ${gameId} - persistent disconnection detected, pausing`
        );
        pauseGame(gameId);
        return;
      }

      // Skip this frame but don't pause yet
      return;
    }

    // Reset disconnection tracking if both players are connected
    if (currentSession.disconnectionWarning) {
      console.log(`Game ${gameId} - connection restored`);
      currentSession.disconnectionWarning = false;
      currentSession.disconnectionCount = 0;
    }
  ...
  console.log(`Game loop successfully started for ${gameId}`);
}
...
function broadcastGameState(gameId) {
  ...
  if (session.player1Sock) {
    p1Sent = sendToPlayer(session.player1Sock, updateMessage);
  }

  if (session.player2Sock) {
    p2Sent = sendToPlayer(session.player2Sock, updateMessage);
  }

  // Log connection status for debugging
  console.log(`Game ${gameId} broadcast - P1: ${p1Sent}, P2: ${p2Sent}`);
}
...
function sendToPlayer(socket, message) {
  if (!socket) {
    console.warn("Cannot send message - socket is null/undefined");
    return false;
  }
...
// Fix the handleReconnection function to properly start the game
function handleReconnection(connection, playerId) {
  ...
  for (const [gameId, session] of sessions.entries()) {
    if (session.playerId1 === playerId || session.playerId2 === playerId) {
      ...
      // Send reconnection message immediately
      connection.send(
        JSON.stringify({
          type: "reconnection",
          gameId: gameId,
      ...
      // Wait a moment for connection to stabilize before checking game start
      setTimeout(() => {
        const p1Connected =
        ...
        console.log(
          `Game ${gameId} delayed connection check: P1=${p1Connected}, P2=${p2Connected}, gameStarted=${session.gameStarted}, waitingForPlayers=${session.waitingForPlayers}`
        );

        if (p1Connected && p2Connected && !session.gameStarted) {
          console.log(
            `Starting game ${gameId} after reconnection - both players connected`
          );
          session.gameStarted = true;
          session.waitingForPlayers = false;

          // Clear any disconnection tracking
          session.disconnectionWarning = false;
          ...
          sendToPlayer(session.player1Sock, startMessage);
          sendToPlayer(session.player2Sock, startMessage);

          // Also send initial game state
          broadcastGameState(gameId);
        }
      }, 100); // 100ms delay to let connection stabilize
...
fastify.get("/ws", { websocket: true }, (connection, req) => {
  ...
  console.log(
    `Player ${playerId} connected via WebSocket, readyState: ${connection.readyState}`
  );
    ...
    try {
      ...
      if (message.type === "move") {
        ...
        if (connection === session.player1Sock) {
          ...
          console.log("Connection not recognized as player1 or player2");
          return;
        }
        ...
        if (player) {
          ...
          // Always broadcast state after paddle movement
  ...
  connection.on("close", (code, reason) => {
    console.log(`Player ${playerId} WebSocket closed: ${code} - ${reason}`);
